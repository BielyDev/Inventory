...
func _ready() -> void:
	var item_instance = load(Inventory.save_dat.item_void.path).instance()
...
func refresh() -> void: # Verifica se o item ainda existe e a quantidade.
	if Inventory.save_dat.item_void != null:
		amount_text.text = str(Inventory.save_dat.item_void.amount)
		if Inventory.save_dat.item_void.amount == 0:
